...
class MyPageDonatedProjectAdapter(
	private val onItemClick: (Int) -> Unit
) : RecyclerView.Adapter<MyPageDonatedProjectAdapter.MyViewHolder>() {

	var dataList = mutableListOf<MyProjectOrderData>()

	inner class MyViewHolder(private val binding: ItemCategoryProjectBinding) :
		RecyclerView.ViewHolder(binding.root) {
		fun bind(projectData: MyProjectOrderData) {
